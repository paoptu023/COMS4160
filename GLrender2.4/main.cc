...
void init() {
    ...
    coarse = glGetUniformLocation(program, "coarseness");
...
void mykey(unsigned char key, int mousex, int mousey) {
    ...
    // decreases detail
    if (key == 'g' && coarseness > 1.0) {
        coarseness -= 1.0;
        glutPostRedisplay();
    }
    
    // increases detail
    if (key == 't' && coarseness < 64.0) {
        coarseness += 1.0;
        glutPostRedisplay();
    }
...
//        read_bezier_file(argv[1], surfaces);
