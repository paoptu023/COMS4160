...
Bezier::Bezier(std::vector<point4> &pnts, int u, int v){
    u_deg = u;
    v_deg = v;
...
void Bezier::sample(int sample_level, std::vector<point4> &uv_verts, std::vector<vec4> &uv_norms){
    if(sample_level >= 2){
...
void Bezier::evaluate(float t_u, float t_v, point4 &point, vec4 &norm){
    std::vector<point4> res_vctrs;
    vec4 tmp;
    for (int i = 0; i <= v_deg; ++i) {
        std::vector<point4> ctr_row = getCtrRow(i);
        point4 new_ctr;
    ...
    std::vector<point4> res_uctrs;
    for (int i = 0; i <= u_deg; ++i) {
        std::vector<point4> ctr_col = getCtrCol(i);
        point4 new_ctr;
 ...
 */
void Bezier::eval_bez(std::vector<point4> &array, const double t, point4 &point, vec4 &tan){
